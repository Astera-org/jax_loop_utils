    ...
    def __init__(
        ...
        if experiment:
            ...
            try:
                ...
                if not experiment:
                    raise RuntimeError(
                        f"Failed to get, then failed to create, then failed to get again experiment '{experiment_name}'"
                    ) from None
                experiment_id = experiment.experiment_id
        self._run_id = self._client.create_run(experiment_id=experiment_id, run_name=run_name).info.run_id

    def write_scalars(self, step: int, scalars: Mapping[str, Scalar]):
        """Write scalar metrics to MLflow."""
        timestamp = int(time.time() * 1000)
        metrics_list = [mlflow.entities.Metric(k, float(v), timestamp, step) for k, v in scalars.items()]
