...
class ReportProgressTest(parameterized.TestCase):
    ...
    def test_named(self, wait_jax_async_dispatch, mock_time):
        ...
        self.assertEqual(
            logs.output,
            ["INFO:absl:Setting work unit notes: 0.2 steps/s, 20.0% (2/10), ETA: 0m (0m : 50.0% test1, 25.0% test2)"],
        )
...
class PeriodicCallbackTest(absltest.TestCase):
    def test_every_steps(self):
        callback = mock.Mock()
        hook = periodic_actions.PeriodicCallback(every_steps=2, callback_fn=callback)
    ...
    def test_function_without_step_and_time(self):
        ...
        hook = periodic_actions.PeriodicCallback(every_steps=1, callback_fn=cb, pass_step_and_time=False)
        hook(0)
        hook(1)
